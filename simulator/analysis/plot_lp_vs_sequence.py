...
def load_index_to_node(
    ...
    if "index_to_node" in raw_data:
        raw_mapping = raw_data["index_to_node"]

        return {
            int(index): str(node_name)
            for index, node_name in raw_mapping.items()
        }

    if "node_to_index" in raw_data:
        return {
            int(index): str(node_name)
            for node_name, index
            in raw_data["node_to_index"].items()
        }

    raise KeyError(
        "topology_db.json contains neither "
        "'index_to_node' nor 'node_to_index'."
    )
...
def plot_network_delivery_and_lp_feasibility(
    sequence_runs: list[dict[str, Any]],
    lp_rows: list[dict[str, Any]],
    output_dir: Path,
) -> None:
    """
    Main saturation graph.

    SeQUeNCe: offered delivery ratio and completion probability.
    LP: binary feasibility boundary.
    """
    sequence_rho = [run["rho"] for run in sequence_runs]
    delivery_ratio = [
        float(run["summary"].get("offered_delivery_ratio", 0.0))
        for run in sequence_runs
    ]
    completion_probability = [
        float(run["summary"].get("completion_probability", 0.0))
        for run in sequence_runs
    ]

    lp_rho = [row["rho"] for row in lp_rows]
    lp_feasible = [1.0 if row["feasible"] else 0.0 for row in lp_rows]
    ...
    ax.plot(
        ...
        delivery_ratio,
        marker="o",
        label="SeQUeNCe delivery ratio",
    )
    ax.plot(
        sequence_rho,
        completion_probability,
        marker="s",
        label="SeQUeNCe completion probability",
    )
    ax.step(
        lp_rho,
        lp_feasible,
    ...
    ax.legend()
    ax.set_title("Network saturation: LP feasibility vs SeQUeNCe delivery")
    save_figure(fig, output_dir, "01_saturation_lp_vs_sequence")
...
def plot_throughput_vs_offered_traffic(
    ...
    sequence_rho = [run["rho"] for run in sequence_runs]
    sequence_throughput = [
        float(run["summary"].get("throughput_pairs_per_s", 0.0))
        for run in sequence_runs
    ]
    sequence_offered = [
        float(
    ...
    lp_rho = [row["rho"] for row in lp_rows if row["offered_traffic"] is not None]
    lp_offered = [
        float(row["offered_traffic"])
        for row in lp_rows
        if row["offered_traffic"] is not None
    ]
    ...
    ax.plot(
        ...
        marker="s",
        linestyle="--",
        label="SeQUeNCe offered traffic",
    )
    ax.plot(
        lp_rho,
        lp_offered,
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    ax.set_title("Offered traffic and delivered throughput")
    save_figure(fig, output_dir, "02_throughput_vs_offered_traffic")
...
def plot_flow_throughput_at_rho(
    ...
    """
    Grouped flow-level comparison at one rho.

    LP rate is taken from traffic_load_<rho>_result.json routing entries.
    SeQUeNCe rate is read from flow_statistics.json.
    """
    sequence_run = min(
        sequence_runs,
        key=lambda run: abs(run["rho"] - selected_rho),
    )
    actual_rho = sequence_run["rho"]
    ...
    sequence_flow_records = load_json(flow_file)
    sequence_rates = {
        f"{record['source']}->{record['destination']}": float(
            record.get("throughput_pairs_per_s", 0.0)
        )
        for record in sequence_flow_records
    }

    candidate_files = [
        lp_results_dir / f"traffic_load_{actual_rho:.2f}_result.json",
    ...
    if lp_result_file is None:
        print(
            "[SKIP] No LP result JSON found for flow-level throughput comparison."
        )
        return
    ...
    flows = sorted(set(sequence_rates) | set(lp_rates))
    ...
    ax.bar(
        positions - width / 2,
        [lp_rates.get(flow, 0.0) for flow in flows],
        width,
        label="LP allocated rate",
    )
    ax.bar(
        positions + width / 2,
        [sequence_rates.get(flow, 0.0) for flow in flows],
        width,
    ...
    ax.set_ylabel("Pairs/s")
    ax.set_title(
        rf"Flow-level throughput comparison at $\rho={actual_rho:.2f}$"
    )
    ...
    save_figure(
        fig,
        output_dir,
        f"04_flow_throughput_rho_{actual_rho:.2f}",
    )
...
def plot_protocol_statistics(
    ...
    for run in sequence_runs:
        ...
        for record in records:
            ...
            total_attempts += int(swapping.get("attempts", 0))
            total_successes += int(swapping.get("successes", 0))
    ...
    fig, ax = plt.subplots(figsize=(9, 5.2))
    ax.plot(rhos, attempts, marker="o")
    ax.set_xlabel(r"Traffic load factor $\rho$")
    ax.set_ylabel("Swapping attempts")
    ax.grid(True, alpha=0.3)
    ax.set_title("Total swapping attempts under increasing load")
    save_figure(fig, output_dir, "08_swapping_attempts_vs_rho")

    fig, ax = plt.subplots(figsize=(9, 5.2))
    ax.plot(rhos, np.array(success_probability) * 100.0, marker="o")
    ax.set_xlabel(r"Traffic load factor $\rho$")
    ax.set_ylabel("Swapping success probability (%)")
    ax.grid(True, alpha=0.3)
    ax.set_title("Observed swapping success probability")
    save_figure(fig, output_dir, "09_swapping_success_probability")
...
def plot_lp_objective_and_feasibility(
    lp_rows: list[dict[str, Any]],
    output_dir: Path,
) -> None:
    objective_rows = [
    ...
    if objective_rows:
        fig, ax = plt.subplots(figsize=(9, 5.2))
        ax.plot(
            [row["rho"] for row in objective_rows],
            [float(row["objective"]) for row in objective_rows],
            marker="o",
        )
        ax.set_xlabel(r"Traffic load factor $\rho$")
        ax.set_ylabel("LP objective")
        ax.set_title("LP expected resource-consumption objective")
        ax.grid(True, alpha=0.3)
        save_figure(fig, output_dir, "19_lp_objective_vs_rho")

    fig, ax = plt.subplots(figsize=(9, 4.7))
    ax.step(
        [row["rho"] for row in lp_rows],
        [1 if row["feasible"] else 0 for row in lp_rows],
        where="post",
        marker="o",
    )
    ax.set_xlabel(r"Traffic load factor $\rho$")
    ax.set_ylabel("LP feasibility")
    ax.set_yticks([0, 1])
    ax.set_yticklabels(["Infeasible", "Feasible"])
    ax.set_title("LP feasibility boundary")
    ax.grid(True, alpha=0.3)
    save_figure(fig, output_dir, "20_lp_feasibility_vs_rho")
...
def plot_link_utilization_scatter(
    ...
    for item in lp_result.get(
        ...
        if node_a_index not in index_to_node:
            raise KeyError(
                f"Unknown LP node index: "
                f"{node_a_index}"
            )

        if node_b_index not in index_to_node:
            raise KeyError(
                f"Unknown LP node index: "
                f"{node_b_index}"
            )

        node_a_name = index_to_node[
            node_a_index
        ]

        node_b_name = index_to_node[
            node_b_index
        ]

        key = _canonical_link_name(
            node_a_name,
            node_b_name,
        )

        lp_utilization[key] = float(
            item["utilization"]
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    type=Path,
    required=True,
    help="Path to topology_db.json containing the node-index mapping.",
)
    parser.add_argument(
        "--lp-results-dir",
        type=Path,
...
def main() -> None:
    ...
    index_to_node = load_index_to_node(
    args.topology_db
)
    ...
    plot_network_delivery_and_lp_feasibility(
        sequence_runs=sequence_runs,
        lp_rows=lp_rows,
        output_dir=args.output_dir,
    )
    ...
    plot_lp_objective_and_feasibility(
        lp_rows=lp_rows,
        output_dir=args.output_dir,
    )
    plot_link_utilization_scatter(
    sequence_runs=sequence_runs,
    lp_results_dir=args.lp_results_dir,
    link_capacities_file=(
        args.link_capacities
    ),
    selected_rho=args.selected_rho,
    output_dir=args.output_dir,
